...
${C_BOLD}Scheduling:${C_RESET}
  Add to crontab:  */5 * * * * /opt/hids/hids.sh --once
...
cmd_once() {
    ...
    mod_result=0
    run_module "${SCRIPT_DIR}/modules/mod_history.sh" "Health History" || mod_result=$?
    [[ "${mod_result}" -gt "${overall_worst}" ]] && overall_worst="${mod_result}"
    ...
    [[ "${mod_result}" -gt "${overall_worst}" ]] && overall_worst="${mod_result}"
    run_module "${SCRIPT_DIR}/modules/mod_network_scan.sh" "Network Scan" || mod_result=$?
