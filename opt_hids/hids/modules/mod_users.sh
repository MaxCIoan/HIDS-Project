...
check_current_sessions() {
    ...
    while IFS= read -r line; do
        ...
        local status="OK"
        # Session physique locale (login, :0, seat0) = toujours OK
        if [[ "${host}" == "login" || "${host}" == ":0" || "${host}" == "seat0" || -z "${host}" ]]; then
            status="OK"
        elif [[ -n "${TRUSTED_SSH_SOURCES}" && -n "${host}" ]]; then
            local trusted=0
...
check_sudo_activity() {
    ...
    local sudo_failures
    sudo_failures=$(echo "${sudo_lines}" | grep "authentication failure\|incorrect password\|user NOT in sudoers" || true)
    local fail_count=0
