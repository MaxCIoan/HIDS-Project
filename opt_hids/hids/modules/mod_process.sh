...
check_resource_hogs() {
    ...
    rows+=("$(gum style --bold --foreground 212 \
        "$(printf '%-8s %-16s %-8s %-8s %-6s %s' 'PID' 'NAME' 'CPU%' 'MEM%' 'USER' 'PATH')")")
    rows+=("$(gum style --foreground 240 \
        "$(printf '%-8s %-16s %-8s %-8s %-6s %s' '────────' '────────────────' '────────' '────────' '──────' '────')")")

    local has_alert=0
    while IFS= read -r line; do
        local pid cpu mem cmd user exe status color
        ...
        rows+=("$(gum style --foreground "${color}" \
            "$(printf '%-8s %-16s %-8s %-8s %-6s %s' \
                "${pid}" "${cmd:0:15}" "${cpu}%" "${mem}%" "${user:0:5}" "${exe:0:40}")")")
