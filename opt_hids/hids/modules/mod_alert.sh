...
send_email_digest() {
    ...
    local warn_count
    warn_count=$(count_by_severity WARN)
    [[ "${crit_count}" -eq 0 && "${warn_count}" -eq 0 ]] && return
    local subject="[HIDS ALERT] ${crit_count} critical / ${warn_count} warning(s) on ${_HIDS_HOST}"
    local body
    body=$(
        printf 'HIDS Alert Digest\n'
        printf '=================\n\n'
        printf 'Host:      %s\n' "${_HIDS_HOST}"
        printf 'Time:      %s\n' "$(now_human)"
        printf 'Critical:  %s\n' "${crit_count}"
