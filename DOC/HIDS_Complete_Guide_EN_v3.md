...
16. [Known Bug Fixes (v2)](#known-bug-fixes-v2)
...
# NEW in v2: exclude auto-updated files from recent modification detection
...
- Top 10 CPU/RAM consumers with full enriched details **(ENHANCED in v2):**
  ...
  - **↳ exe** — full binary path from `/proc/[pid]/exe`
...
### Module 6 — Network Scan (mod_network_scan.sh) 🔧 Fixed in v2

**What it monitors:** **All** active hosts on lab-net (auto-discovered), port changes on every monitored host, established connections.

**v2 fix:** The scan now auto-discovers ALL hosts on `192.168.0.0/24` and creates
a separate port baseline per host. Previously only Metasploitable was scanned.

**Data sources:**
- Host discovery → `nmap -sn 192.168.0.0/24` (all hosts, excludes our own IP)
- Port scan → `nmap -sT --open -p-` (per discovered host)
- Per-host baselines → `/var/lib/hids/network_baseline/[IP]_ports.list`
...
- Subject: `[HIDS ALERT] N critical / N warning(s) on hostname`
- Content: host, timestamp, counts, and full details of every finding
...
| `cups/subscriptions.conf` modified | CUPS updates this file automatically | `INTEGRITY_RECENT_EXCLUDE="/etc/cups"` **(NEW v2)** |
...
| Email only on CRITICAL | WARN alerts were not emailed | `send_email_digest()` updated to send WARN too |
...
*Last updated: April 16, 2026 — v2 changes: mod_history, enhanced mod_process, DISK_EXCLUDE_MOUNTPOINTS, INTEGRITY_RECENT_EXCLUDE, email WARN, network scan all hosts, bug fixes*
